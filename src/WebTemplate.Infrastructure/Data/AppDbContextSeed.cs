...
public class AppDbContextSeed
{
    ...
    public async Task Seed()
    {
        ...
        await _db.Set<User>().AddAsync(new User { UserName = "super", NormalizedUserName = "super".Normalized(), SecurityStamp = salt, PasswordHash = _passwodHasher.HashPassword("123456", salt) }).ConfigureAwait(false);
        await _db.Set<User>().AddAsync(new User { UserName = "admin", NormalizedUserName = "admin".Normalized(), SecurityStamp = salt, PasswordHash = _passwodHasher.HashPassword("123456", salt) }).ConfigureAwait(false);
        await _db.Set<User>().AddAsync(new User { UserName = "user", NormalizedUserName = "user".Normalized(), SecurityStamp = salt, PasswordHash = _passwodHasher.HashPassword("123456", salt) }).ConfigureAwait(false);
