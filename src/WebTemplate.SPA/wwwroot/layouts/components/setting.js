...
export default {
  template: html`<el-drawer title="页面配置" v-model="appStore.showSettingPanel">
    <el-form ref="formRef" :model="appStore" label-width="auto">
      ...
      <el-form-item :prop="appStore.locale.current" label="默认语言">
        <el-select v-model="appStore.locale.current">
          <template v-for="item in appStore.locale.options" :key="item.value">
            <el-option :value="item.value" :label="item.label"> <span>{{item.label}}</span></el-option>
          </template>
        </el-select>
      </el-form-item>
      <el-form-item :prop="appStore.size" label="组件大小">
        <el-select v-model="appStore.size">
          <template v-for="item in sizeOptions" :key="item.value">
            <el-option :value="item.value" :label="item.text"> <span>{{item.text}}</span></el-option>
          </template>
        </el-select>
      </el-form-item>
